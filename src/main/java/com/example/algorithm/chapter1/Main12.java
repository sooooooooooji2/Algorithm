...
public class Main12 {
    public static void main(String[] args) throws IOException {
        ...
        String inputStr = br.readLine();        //문자열 받아온거

        String binaryStr = "";  //이진수값
        String result = ""; //최종 리턴 값

        for(int i = 0 ; i < inputNum; i++){
            //1. “#*****#”를 일곱자리의 이진수로 바꿉니다. #은 이진수의 1로, *이진수의 0으로 변환합 니다. 결과는 “1000001”로 변환됩니다.
            binaryStr = inputStr.substring(0, 7).replace('#','1').replace('*','0');

            //2. 바뀐 2진수를 10진수화 합니다. “1000001”을 10진수화 하면 65가 됩니다.
            int binaryInt = Integer.parseInt(binaryStr, 2); // 2진수는 Integer.parseInt(binaryStr, 2)

            //3. 아스키 번호가 65문자로 변환합니다. 즉 아스크번호 65는 대문자 'A'입니다.
            result += (char)binaryInt;

            //4. 받아온 문자열을 다시 정비
            inputStr = inputStr.substring(7);      //substring()
        }
